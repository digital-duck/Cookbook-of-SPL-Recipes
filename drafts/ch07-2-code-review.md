...
The Code Review recipe (Recipe 15) implements this multi-pass strategy. It scores each category for "Severity" and uses those scores to decide whether to `approve`, `request_changes`, or `block` the code—bringing the discipline of a senior engineer to an automated pipeline.
