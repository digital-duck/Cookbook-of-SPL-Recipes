...
What makes this recipe different from section 2.1.1 (RAG Query) is the *conversation loop*. section 2.1.1 answers a single question. This recipe maintains a conversation — tracking what has been asked and answered, so follow-up questions like "What does that mean in practice?" resolve correctly.
...
**Use section 2.1.1 (RAG Query) instead when:**
- You have a single question, not a conversation
- You are querying across many documents (section 2.1.1's corpus indexing model scales better)
