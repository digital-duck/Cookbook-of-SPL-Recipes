...
<!-- *"The simplest tool is often the most powerful when it becomes a building block." — Wen Gong* -->
...
We trigger the LLM call. Unlike section 1.1, we now pass an argument to the step: `answer(prompt)`. This tells the runtime to specifically emphasize the `prompt` column from our `SELECT` statement as the primary instruction for the model.
