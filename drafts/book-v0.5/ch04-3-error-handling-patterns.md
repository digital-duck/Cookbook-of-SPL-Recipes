...
3. **Write a validator.** For section 1.2.3 (Plan and Execute), add a validation step using a second GENERATE call that checks whether the generated plan is well-formed (numbered steps, at least 3 steps, no duplicate step descriptions). Route `invalid` plans back to the planner for a retry, up to 2 retries.
