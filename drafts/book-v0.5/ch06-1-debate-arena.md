...
<!-- *"The truth is a spark that flies when two stones of opinion are struck together." — Wen Gong* -->
...
The Debate Arena recipe (section 6.1) implements a multi-round debate. Two personas trade opening statements and rebuttals, while a third "Judge" persona monitors the history and declares a winner. This is a powerful technique for stress-testing ideas and discovering edge cases.
...
DO
    ...
    COMMIT @verdict WITH status = 'complete', rounds = @round   -- (5) Final Judgment
END
```
...
Notice `@pro_history || '\n---\n' || @pro_rebuttal`. The `||` operator is SPL's unambiguous string concatenation — borrowed from Oracle/PostgreSQL SQL. `+` is ambiguous (it also performs numeric addition). Use `||` for strings, `+` for numbers.

### (5) LOGGING for Progress Visibility

The `LOGGING` statements are not decoration. In a multi-round debate, you want to see which round is running and when the judge starts deliberating — especially on a GTX 1080 Ti where each rebuttal takes several seconds. `LEVEL INFO` shows phase transitions; `LEVEL DEBUG` shows per-round detail.
