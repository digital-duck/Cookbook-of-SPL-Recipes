...
<!-- *"Knowledge is not what you have in your head, but knowing where to find it." — Wen Gong* -->
...
The RAG Query recipe (section 5.1) demonstrates the simplest possible RAG implementation in SPL. It uses the built-in `rag.query` function to perform a semantic search against a local vector database and feeds the results directly into the prompt.
