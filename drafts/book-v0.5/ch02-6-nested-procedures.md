...
<!-- *"Complexity should be composed, not managed." — Wen Gong* -->
...
The Nested Procedures recipe (section 2.6) demonstrates this hierarchy. It builds a "Layered Explainer" where high-level orchestration is separated from low-level generation tasks like complexity calibration and example making.
...
The `CALL` statement is the bridge. In SPL 2.0, `CALL` is used for both Python tools (section 2.2) and other SPL procedures. This unified interface means you can swap a generative procedure for a deterministic tool without changing the calling code.
