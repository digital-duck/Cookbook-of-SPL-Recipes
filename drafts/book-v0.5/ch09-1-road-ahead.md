...
## The Type Progression: Why "Simple" Works Forever

The 37 recipes in this book all work with text. That is where AI workflows live in 2026. But the language is designed to stay simple as the type system grows.

Here is the full progression, where it is now, and where it is going:

| Version | Types | Era |
|---------|-------|-----|
| SPL v1.0 | `TEXT` | Prompt engineering |
| SPL v2.0 | `TEXT`, `NUMBER`, `BOOL`, `LIST` | Agentic workflows |
| SPL v3.0 | + `IMAGE`, `AUDIO`, `VIDEO` | Multimodal AI |
| SPL v4.0 | + `STREAM`, `GRAPH`, `SENSOR` | Physical world — IoT, camera feeds, real-time data |

In each version, the *language* stays the same. `WHILE`, `EVALUATE`, `CALL`, `GENERATE`, `EXCEPTION` — unchanged. The *world it can describe* grows larger.

A v2.0 workflow that calls `GENERATE summarize(@text)` becomes a v3.0 workflow that calls `GENERATE describe_image(@photo)` by changing one type declaration and one function name. The control flow, the CALL/GENERATE split, the adapter abstraction — none of it changes.

This is why "Simple" is the right name and not a temporary one. A language named "Structured Prompt Language" would have aged out the moment multimodal arrived — prompts are text, and text is just one modality. "Simple Programming Language" makes no such promise about modality. It promises only one thing: **that expressing what you want should never be harder than the thing itself.**

That promise holds for text. It holds for images. It holds for audio, video, and sensor streams. Simplicity is not a feature — it is the design commitment that all features are measured against.

```spl
-- v3.0: the workflow looks familiar
...
Same grammar. New type. Zero relearning.
...
**If you are in the Global South** with commodity hardware: set up Ollama, install `spl-llm`, and run section 1.1.1. The stack runs on a GTX 1080 Ti. If that is more than you have, try section 1.1.1 with a smaller model. The barrier to entry is a `pip install`, not a credit card.
