...
The Reflection Agent recipe (section 2.4) implements this loop using a `WHILE` block. It doesn't just refine the text; it calculates a "Confidence Score." If the score is too low, it iterates. If the score is high, it stops. This ensures that the workflow is only as complex as the problem requires.
