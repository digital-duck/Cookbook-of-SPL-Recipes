...
## SPL and Momagrid: Two Different Things

Before going further, a clarification that will save confusion later.

**SPL is the language.** It is what you write: `.spl` files containing `WORKFLOW`, `GENERATE`, `CALL`, and the other constructs in this book. SPL runs on your laptop with Ollama, on any cloud API, or on any machine with Python. You do not need Momagrid to use SPL.

**Momagrid is an optional distributed runtime.** It is a globally distributed mesh of compute nodes — cloud, edge, and peer-to-peer — on which SPL workflows can execute without modification. The same `.spl` file that runs locally on your GTX 1080 Ti runs on Momagrid unchanged. Momagrid unlocks the decentralized vision: distributed LLM inference, compute-to-data dispatch, and resilience by design.

You can use SPL without Momagrid indefinitely. You can run all 37 recipes in this book on a single local machine. Momagrid is where SPL goes when you want global scale and decentralized infrastructure — but it is not a prerequisite for anything in this book.

*Think of it this way: SQL is the language. PostgreSQL (or MySQL, or DuckDB) is the runtime. You learn SQL once; you choose the runtime that fits your deployment.*
