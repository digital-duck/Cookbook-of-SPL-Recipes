...
<!-- *"The real world is messy; your data doesn't have to be." — Wen Gong* -->
...
The Data Extraction recipe (section 8.6) implements this pattern. It uses a `CREATE FUNCTION` block to deliver different extraction schemas based on a `format` parameter (e.g., `invoice`, `contract`, or `general`). This allows a single workflow to handle a wide variety of business documents with high precision.
