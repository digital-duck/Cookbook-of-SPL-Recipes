...
Every SQL database ships with a standard library of functions — `UPPER`, `ROUND`, `COALESCE`, `JSON_GET` — so practitioners never need to rewrite the same utility logic. SPL 2.0 follows the same philosophy. The SPL Standard Library provides built-in functions available in every workflow without any `--tools` flag or import statement.
...
SPL 2.0 has a native `LIST` type. `[]` creates an empty list; the `list_*` functions operate on it. Internally lists are JSON arrays — no delimiter ambiguity, no split-on-comma surprises.

| Function | Description |
|---|---|
| `list_append(list, item)` | Append an item; returns updated list |
| `list_concat(list, separator)` | Join all elements into a single TEXT string |
| `list_count(list)` | Number of elements |
| `list_get(list, index)` | 0-based element access |

```spl
@items := []
@items := list_append(@items, 'first')
@items := list_append(@items, 'second')
@n     := list_count(@items)                     -- '2'
@text  := list_concat(@items, ', ')              -- 'first, second'
@item  := list_get(@items, 0)                    -- 'first'
```

The map-reduce pattern (Chapter 7.1) is the canonical use: accumulate chunk summaries into a LIST during the Map phase, then `list_concat` before the Reduce GENERATE call.

<!-- --- -->

## File I/O

`write_file` and `read_file` are built-in — no `--tools` flag, no `tools.py` entry needed. They are deterministic `CALL` operations: zero LLM tokens, always.

| Function | Description |
|---|---|
| `write_file(path, content)` | Write content to file; creates parent dirs automatically. Returns `'written: <path>'`. |
| `read_file(path)` | Read file contents as TEXT. Returns `''` if file does not exist. |

```spl
-- Save an intermediate result for inspection
CALL write_file(f'{@log_dir}/summary_{@i}.md', @chunk_summary) INTO @_

-- Load a prompt template from disk
CALL read_file('prompts/system_prompt.md') INTO @system_prompt
GENERATE analyze(@doc, @system_prompt) INTO @result
```

`INTO @_` discards the return value — the SPL equivalent of Python's `_ = fn()`. Use it when the side-effect (writing the file) is what you want and the return value is not needed.

File paths are relative to the working directory where `spl run` is invoked. Use the `@log_dir` INPUT parameter pattern to keep paths configurable:

```spl
WORKFLOW my_workflow
    INPUT:
        @document TEXT,
        @log_dir TEXT DEFAULT 'my_workflow/logs'
    OUTPUT: @result TEXT
DO
    -- ... processing ...
    CALL write_file(f'{@log_dir}/result.md', @result) INTO @_
END
```
...
The stdlib covers routine operations including file I/O, list manipulation, string processing, JSON parsing, type conversion, and datetime. When you need domain-specific logic — PII detection, database lookups, external API calls, or custom computation — write a UDF. See Chapter 10.4 for how to extend SPL with your own tools.

The decision rule: if it has a clear, general-purpose definition independent of your domain, it belongs in stdlib. If it requires your schema, your credentials, or your business rules, it belongs in a `tools.py` UDF.
