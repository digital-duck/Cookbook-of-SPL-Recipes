...
# Local Ollama
spl run src/recipes/ch02-3-plan-and-execute/plan_execute.spl --adapter ollama \
    task="Build a REST API for a todo app"

# Momagrid
spl run src/recipes/ch02-3-plan-and-execute/plan_execute.spl --adapter momagrid \
    task="Build a REST API for a todo app"

# Generate and write code to disk (one file per LLM call — reliable for any model)
# Local Ollama
spl run src/recipes/ch02-3-plan-and-execute/plan_execute.spl \
    --adapter ollama -m "qwen2.5-coder" \
    --tools src/recipes/ch02-3-plan-and-execute/tools.py \
    task="Build a REST API for a todo app" \
    output_dir="src/recipes/ch02-3-plan-and-execute/output" \
    max_steps=4 \
    2>&1 | tee src/recipes/ch02-3-plan-and-execute/out/12_plan_and_execute-ollama-qwen2.5-$(date +%Y%m%d_%H%M%S).md

# Momagrid
spl run src/recipes/ch02-3-plan-and-execute/plan_execute.spl \
    --adapter momagrid -m "qwen2.5-coder" \
    --tools src/recipes/ch02-3-plan-and-execute/tools.py \
    task="Build a REST API for a todo app" \
    output_dir="src/recipes/ch02-3-plan-and-execute/output" \
    max_steps=4 \
    2>&1 | tee src/recipes/ch02-3-plan-and-execute/out/12_plan_and_execute-momagrid-qwen2.5-$(date +%Y%m%d_%H%M%S).md
...
spl run src/recipes/ch02-3-plan-and-execute/plan_execute.spl \
    ...
    output_dir="src/recipes/ch02-3-plan-and-execute/output" \
    max_steps=4 \
    2>&1 | tee src/recipes/ch02-3-plan-and-execute/out/12_plan_and_execute-claude-sonnet-$(date +%Y%m%d_%H%M%S).md
...
spl run src/recipes/ch02-3-plan-and-execute/plan_execute.spl \
    ...
    output_dir="src/recipes/ch02-3-plan-and-execute/output" \
    max_steps=4 \
    2>&1 | tee src/recipes/ch02-3-plan-and-execute/out/12_plan_and_execute-claude-$(date +%Y%m%d_%H%M%S).md
```
...
```bash
cd src/recipes/ch02-3-plan-and-execute/output-claude-sonnet4-6
...
python3 -m http.server 3001 \
    --directory src/recipes/ch02-3-plan-and-execute/front-end-simple
...
```bash
cd src/recipes/ch02-3-plan-and-execute/front-end-vuejs
npm install
npm run dev
...
`tools.py` provides `write_code_files(content, output_dir)` — parses fenced code blocks with `# filename: path` as the first line and writes each to `output_dir`. Loaded with `--tools src/recipes/ch02-3-plan-and-execute/tools.py`.
