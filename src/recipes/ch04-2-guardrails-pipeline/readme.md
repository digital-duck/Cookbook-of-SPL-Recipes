...
# Local Ollama
spl run src/recipes/ch04-2-guardrails-pipeline/guardrails.spl \
    --adapter ollama -m gemma3 \
    --tools src/recipes/ch04-2-guardrails-pipeline/tools.py \
    input_id=safe_encryption \
    2>&1 | tee src/recipes/ch04-2-guardrails-pipeline/out/18_guardrails-$(date +%Y%m%d_%H%M%S).md
...
spl run src/recipes/ch04-2-guardrails-pipeline/guardrails.spl \
    ...
    --tools src/recipes/ch04-2-guardrails-pipeline/tools.py \
    input_id=safe_encryption \
    2>&1 | tee src/recipes/ch04-2-guardrails-pipeline/out/18_guardrails-momagrid-$(date +%Y%m%d_%H%M%S).md
