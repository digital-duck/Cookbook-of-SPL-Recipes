...
```bash
# Local Ollama
spl run src/recipes/ch07-2-code-review/code_review.spl --adapter ollama \
    code="$(cat digest.go)" \
    language="Go" \
    2>&1 | tee src/recipes/ch07-2-code-review/out/15_code_review-$(date +%Y%m%d_%H%M%S).md
...
spl run src/recipes/ch07-2-code-review/code_review.spl --adapter ollama \
    code="$(cat main.py)" \
    language="Python" \
    2>&1 | tee src/recipes/ch07-2-code-review/out/15_code_review-python-$(date +%Y%m%d_%H%M%S).md

# Momagrid
spl run src/recipes/ch07-2-code-review/code_review.spl --adapter momagrid \
    code="$(cat digest.go)" \
    language="Go" \
    2>&1 | tee src/recipes/ch07-2-code-review/out/15_code_review-momagrid-$(date +%Y%m%d_%H%M%S).md
...
spl run src/recipes/ch07-2-code-review/code_review.spl --adapter momagrid \
    code="$(cat main.py)" \
    language="Python" \
    2>&1 | tee src/recipes/ch07-2-code-review/out/15_code_review-momagrid-python-$(date +%Y%m%d_%H%M%S).md
```
