...
# Local Ollama
spl run src/recipes/ch07-9-sentiment-pipeline/sentiment.spl \
    --adapter ollama -m gemma3 \
    --tools src/recipes/ch07-9-sentiment-pipeline/tools.py \
    filename=product_reviews.txt \
    domain=product_reviews \
    2>&1 | tee src/recipes/ch07-9-sentiment-pipeline/out/31_sentiment-$(date +%Y%m%d_%H%M%S).md
...
spl run src/recipes/ch07-9-sentiment-pipeline/sentiment.spl \
    ...
    filename=product_reviews.txt \
    domain=product_reviews \
    2>&1 | tee src/recipes/ch07-9-sentiment-pipeline/out/31_sentiment-momagrid-$(date +%Y%m%d_%H%M%S).md
