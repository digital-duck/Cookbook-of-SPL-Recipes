...
```bash
# Local Ollama
spl run src/recipes/ch04-1-safe-generation/safe_generation.spl --adapter ollama \
    -m phi4 prompt="Explain how internet works" \
    2>&1 | tee src/recipes/ch04-1-safe-generation/out/07_safe_generation-$(date +%Y%m%d_%H%M%S).md

spl run src/recipes/ch04-1-safe-generation/safe_generation.spl --adapter ollama \
    prompt="What are the trade-offs of microservices?"

# Momagrid
spl run src/recipes/ch04-1-safe-generation/safe_generation.spl --adapter momagrid \
    -m phi4 prompt="Explain how internet works" \
    2>&1 | tee src/recipes/ch04-1-safe-generation/out/07_safe_generation-momagrid-$(date +%Y%m%d_%H%M%S).md

spl run src/recipes/ch04-1-safe-generation/safe_generation.spl --adapter momagrid \
    prompt="What are the trade-offs of microservices?"

spl run src/recipes/ch04-1-safe-generation/safe_generation.spl --adapter claude_cli \
