# ch09-2: Recipe Maker (Meta-Recipe)
...
# Local Ollama
spl run src/recipes/ch09-2-recipe-maker/recipe_maker.spl \
    --adapter ollama -m gemma3 \
    --tools src/recipes/ch09-2-recipe-maker/tools.py \
    concept="Customer churn predictor with SHAP explainability" \
    output_dir="src/recipes/ch09-2-recipe-maker/output/38_churn_predictor/"

# Momagrid
spl run src/recipes/ch09-2-recipe-maker/recipe_maker.spl \
    --adapter momagrid -m gemma3 \
    --tools src/recipes/ch09-2-recipe-maker/tools.py \
    concept="Customer churn predictor with SHAP explainability" \
    output_dir="src/recipes/ch09-2-recipe-maker/output/38_churn_predictor/"
...
spl run src/recipes/ch09-2-recipe-maker/recipe_maker.spl \
    ...
    output_dir="src/recipes/ch09-2-recipe-maker/output/39_resume_screener/"
...
spl run src/recipes/ch09-2-recipe-maker/recipe_maker.spl \
    ...
    output_dir="src/recipes/ch09-2-recipe-maker/output/40_contract_risk/"
...
spl run src/recipes/ch09-2-recipe-maker/recipe_maker.spl \
    ...
    dataset="src/recipes/ch07-6-support-triage/orders.json" \
    output_dir="src/recipes/ch07-6-support-triage-v2/"
...
**Start with a recipe you already know.** Recreating an existing cookbook recipe is the ideal first test — you have ground truth to compare against and know exactly what a good result looks like. Try `ch07-6-support-triage` or `ch07-7-meeting-actions` first (simple, well-defined), then attempt something more complex like `ch06-5-interview-sim` to see how the recipe-maker handles multi-persona structure and scoring rubrics.
