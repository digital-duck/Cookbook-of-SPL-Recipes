# ch07-3: Headline News Aggregator
...
```bash
# Local Ollama
spl run src/recipes/ch07-3-headline-news/headline_news.spl --adapter ollama -m phi4 \
    topic="artificial intelligence" \
    2>&1 | tee src/recipes/ch07-3-headline-news/out/37_headline_news-ai-ollama_phi4-$(date +%Y%m%d_%H%M%S).md

# Momagrid
spl run src/recipes/ch07-3-headline-news/headline_news.spl --adapter momagrid -m phi4 \
    topic="artificial intelligence" \
    2>&1 | tee src/recipes/ch07-3-headline-news/out/37_headline_news-ai-momagrid_phi4-$(date +%Y%m%d_%H%M%S).md

# Default — balanced structured digest on AI
spl run src/recipes/ch07-3-headline-news/headline_news.spl --adapter claude_cli -m claude-opus-4-6 \
    topic="artificial intelligence" \
    2>&1 | tee src/recipes/ch07-3-headline-news/out/37_headline_news-ai-$(date +%Y%m%d_%H%M%S).md
...
spl run src/recipes/ch07-3-headline-news/headline_news.spl --adapter claude_cli -m claude-opus-4-6 \
    ...
    max_headlines=5 \
    2>&1 | tee src/recipes/ch07-3-headline-news/out/37_headline_news-energy-$(date +%Y%m%d_%H%M%S).md
...
spl run src/recipes/ch07-3-headline-news/headline_news.spl --adapter claude_cli -m claude-opus-4-6 \
    ...
    perspective="global" \
    2>&1 | tee src/recipes/ch07-3-headline-news/out/37_headline_news-quantum-$(date +%Y%m%d_%H%M%S).md
...
spl run src/recipes/ch07-3-headline-news/headline_news.spl --adapter claude_cli -m claude-opus-4-6 \
    ...
    format="narrative" \
    perspective="policy" \
    2>&1 | tee src/recipes/ch07-3-headline-news/out/37_headline_news-semicon-$(date +%Y%m%d_%H%M%S).md
```
...
- Adding a `tool_use` call (ch10-4 pattern) to a search API (DuckDuckGo, NewsAPI, Brave Search)
...
| Extension | How |
|---|---|
| Real-time news | Add `CALL search_web(@topic)` via tool_use (ch10-4 pattern) before Step 1 |
| Gemini grounding | Replace `--adapter claude_cli` with `--adapter gemini` for Google Search-grounded output |
| Multi-topic digest | Wrap in a `FOR` loop over a list of topics (ch07-10 Progressive Summary pattern) |
