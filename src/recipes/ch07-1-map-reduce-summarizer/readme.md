...
# Local Ollama
spl run src/recipes/ch07-1-map-reduce-summarizer/map_reduce.spl --adapter ollama \
    -m gemma3 --tools src/recipes/ch07-1-map-reduce-summarizer/tools.py \
    document="$(cat src/recipes/ch07-1-map-reduce-summarizer/large_doc.txt)" \
    style="bullet points" \
    2>&1 | tee src/recipes/ch07-1-map-reduce-summarizer/out/13_map_reduce-long-$(date +%Y%m%d_%H%M%S).md

# Momagrid
spl run src/recipes/ch07-1-map-reduce-summarizer/map_reduce.spl --adapter momagrid \
    -m gemma3 --tools src/recipes/ch07-1-map-reduce-summarizer/tools.py \
    document="$(cat src/recipes/ch07-1-map-reduce-summarizer/large_doc.txt)" \
    style="bullet points" \
    2>&1 | tee src/recipes/ch07-1-map-reduce-summarizer/out/13_map_reduce-momagrid-long-$(date +%Y%m%d_%H%M%S).md

# Medium document — Markdown format, tests section boundary handling
# Local Ollama
spl run src/recipes/ch07-1-map-reduce-summarizer/map_reduce.spl --adapter ollama \
    document="$(cat src/recipes/ch07-1-map-reduce-summarizer/sample.md)" \
    style="executive brief" \
    2>&1 | tee src/recipes/ch07-1-map-reduce-summarizer/out/13_map_reduce-medium-$(date +%Y%m%d_%H%M%S).md

# Momagrid
spl run src/recipes/ch07-1-map-reduce-summarizer/map_reduce.spl --adapter momagrid \
    document="$(cat src/recipes/ch07-1-map-reduce-summarizer/sample.md)" \
    style="executive brief" \
    2>&1 | tee src/recipes/ch07-1-map-reduce-summarizer/out/13_map_reduce-momagrid-medium-$(date +%Y%m%d_%H%M%S).md
...
spl run src/recipes/ch07-1-map-reduce-summarizer/map_reduce.spl --adapter ollama \
    ...
    style="narrative" \
    2>&1 | tee src/recipes/ch07-1-map-reduce-summarizer/out/13_map_reduce-short-$(date +%Y%m%d_%H%M%S).md

# Momagrid
spl run src/recipes/ch07-1-map-reduce-summarizer/map_reduce.spl --adapter momagrid \
    document="Artificial intelligence has transformed how organizations process and analyze large volumes of unstructured data. Key applications include document summarization, sentiment analysis, and automated report generation." \
    style="narrative" \
    2>&1 | tee src/recipes/ch07-1-map-reduce-summarizer/out/13_map_reduce-momagrid-short-$(date +%Y%m%d_%H%M%S).md
```
