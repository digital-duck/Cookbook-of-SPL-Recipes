...
```bash
bash src/recipes/ch07-0-text2spl-demo/text2spl_demo.sh [adapter] [model]
```

Default adapter: `ollama` or `momagrid`, default model: `gemma3`.
...
Generated `.spl` files are written to `src/recipes/ch07-0-text2spl-demo/generated/` and validated with `spl validate`. To run a generated file:

```bash
# Local Ollama
spl run src/recipes/ch07-0-text2spl-demo/generated/summarize.spl --adapter ollama

# Momagrid
spl run src/recipes/ch07-0-text2spl-demo/generated/summarize.spl --adapter momagrid
```
