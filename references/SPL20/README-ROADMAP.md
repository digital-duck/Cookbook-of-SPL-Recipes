...
## Feedback from v0.1 Editorial Review — Technical Items

*Captured from Opus 4.6 editorial review, March 2026. These items have implications for the SPL runtime, documentation, or cookbook beyond pure editorial work.*

### 1. CALL/GENERATE Split — Explicit Documentation

**Finding:** The CALL/GENERATE distinction (deterministic tool calls vs. probabilistic LLM invocations) is the most valuable design principle in the book. Every recipe implicitly teaches it, but it is never stated as a standalone principle.
...
**Finding:** The book repeatedly promises that the same `.spl` file runs unchanged across adapters (Ollama, Anthropic, OpenRouter, Momagrid). This is the central value proposition. Yet no chapter demonstrates it by running the same recipe across three or four adapters and showing output side-by-side.

**Chapter 9.1 (Model Showdown) comes close** but swaps models *within* a single adapter rather than swapping adapters.

**Action:** Add a chapter titled "One Recipe, Four Adapters" that runs a single meaningful recipe (not Hello World — something with a WORKFLOW, at least two GENERATE calls, and a non-trivial output) against Ollama, Anthropic, and OpenRouter. Show the outputs. Show the latencies. Show what changes (output variation due to model differences) and what doesn't (the `.spl` file is identical). This is the proof that backs up the promise.

**Implementation note:** The recipe should use `--adapter ollama`, `--adapter claude_cli`, and `--adapter openrouter` with the same `.spl` source. The outputs will differ (different models produce different text), but the *structure* and *execution flow* should be identical. Capture this with actual benchmark data.

**Priority:** High. This is the single strongest argument for SPL's architecture — it currently goes undemonstrated.

### 3. Error Handling Coverage

**Finding:** No dedicated recipe teaches what happens when things go wrong in production: malformed JSON from structured-output workflows, Python tools raising exceptions, Ollama server unreachable mid-workflow. The EXCEPTION block is mentioned in Hello World and the recipe-maker, but never the primary topic.

**Status:** *Partially addressed* — `ch04-3-error-handling-patterns.md` was added in v0.2 covering four error handling patterns (graceful degradation, retry with backoff, multi-step isolation, LLM-as-judge validation).

**Remaining gap:** The chapter covers pattern templates but does not show concrete runtime error messages and recovery flows for the three specific failure modes above. Consider adding a "When Things Go Wrong" appendix or expanding ch04-3 with concrete error output examples from actual failed runs.

**Priority:** Medium. The chapter exists; the gap is concrete error trace examples.

### 4. SPL Ecosystem and Adoption Documentation

**Finding:** SPL is not yet widely adopted. A reader investing time in learning SPL is making a bet on the language's future. The book should address this directly rather than implying inevitability.

**Action:** Add an honest section to the closing chapter (ch09-1) covering:
- Current state of the SPL ecosystem: open-source status, GitHub stats, known production deployments (if any), community size
- The roadmap: what is planned for SPL v3, Momagrid scale milestones
- Migration path: if a reader's team evaluates SPL and decides it is not right for them, what do they take away? (The patterns, the mental model, the CALL/GENERATE discipline — these transfer to any framework)
